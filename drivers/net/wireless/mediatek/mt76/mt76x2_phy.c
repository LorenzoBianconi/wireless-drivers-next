...
mt76x2_phy_tssi_init_cal(struct mt76x2_dev *dev)
{
	...
	mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_TSSI, flag);
...
mt76x2_phy_channel_calibrate(struct mt76x2_dev *dev, bool mac_stopped)
{
	...
	if (is_5ghz)
		mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_LC, 0);

	mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_TX_LOFT, is_5ghz);
	mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_TXIQ, is_5ghz);
	mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_RXIQC_FI, is_5ghz);
	mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_TEMP_SENSOR, 0);
	mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_TX_SHAPING, 0);
			   ...
			   struct cfg80211_chan_def *chandef)
{
	...
	if (!dev->cal.init_cal_done) {
		...
		if (val != 0xff)
			mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_R, 0);
	}

	mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_RXDCOC, channel);

	/* Rx LPF calibration */
	if (!dev->cal.init_cal_done)
		mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_RC, 0);
...
mt76x2_phy_tssi_compensate(struct mt76x2_dev *dev)
{
	...
	if (!dev->cal.tssi_comp_pending) {
		...
		mt76xx_mcu_calibrate(&dev->mt76, MCU_CAL_DPD, chan->hw_value);
