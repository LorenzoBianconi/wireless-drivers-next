...
({									\
	mt7921_seq_puts_array(s, #prefix " (user)", txpwr.data[TXPWR_USER].rate, \
			    ARRAY_SIZE(txpwr.data[TXPWR_USER].rate)); \
	mt7921_seq_puts_array(s, #prefix " (eeprom)", txpwr.data[TXPWR_EEPROM].rate, \
			    ARRAY_SIZE(txpwr.data[TXPWR_EEPROM].rate));	\
	mt7921_seq_puts_array(s, #prefix " (tmac)", txpwr.data[TXPWR_MAC].rate,	\
...
static int
mt7921_config(void *data, u64 val)
{
	struct mt7921_dev *dev = data;
	int ret;
