...
static int
mt7921_tm_set_tx_power(struct mt7921_phy *phy)
{
...
static int
mt7921_tm_set_freq_offset(struct mt7921_dev *dev, bool en, u32 val)
{
	struct mt7921_tm_cmd req = {
		.testmode_en = en,
...
static int
mt7921_tm_mode_ctrl(struct mt7921_dev *dev, bool enable)
{
	struct {
		u8 format_id;
		bool enable;
...
static int
mt7921_tm_set_trx(struct mt7921_dev *dev, struct mt7921_phy *phy,
		  int type, bool en)
{
	struct mt7921_tm_cmd req = {
		.testmode_en = 1,
...
static void
mt7921_tm_reg_backup_restore(struct mt7921_dev *dev, struct mt7921_phy *phy)
{
...
static void
mt7921_tm_init(struct mt7921_dev *dev)
{
	...
	mt7921_tm_mode_ctrl(dev, en);
	mt7921_tm_reg_backup_restore(dev, &dev->phy);
	mt7921_tm_set_trx(dev, &dev->phy, TM_MAC_TXRX, !en);
}

static void
mt7921_tm_set_tx_frames(struct mt7921_dev *dev, bool en)
{
	...
	mt7921_tm_set_trx(dev, &dev->phy, TM_MAC_RX_RXV, false);

	if (en) {
		u8 tx_ant = dev->mt76.test.tx_antenna_mask;

		mutex_unlock(&dev->mt76.mutex);
		mt7921_set_channel(&dev->phy);
		mutex_lock(&dev->mt76.mutex);

		mt7921_mcu_set_chan_info(&dev->phy, MCU_EXT_CMD_SET_RX_PATH);
		dev->test.spe_idx = spe_idx_map[tx_ant];
	}

	mt7921_tm_set_trx(dev, &dev->phy, TM_MAC_TX, en);
...
static void
mt7921_tm_set_rx_frames(struct mt7921_dev *dev, bool en)
{
	if (en) {
		mutex_unlock(&dev->mt76.mutex);
		mt7921_set_channel(&dev->phy);
	...
	mt7921_tm_set_trx(dev, &dev->phy, TM_MAC_RX_RXV, en);
}

static void
mt7921_tm_update_params(struct mt7921_dev *dev, u32 changed)
{
	struct mt76_testmode_data *td = &dev->mt76.test;
	bool en = dev->mt76.test.state != MT76_TM_STATE_OFF;

	if (changed & BIT(TM_CHANGED_FREQ_OFFSET))
		mt7921_tm_set_freq_offset(dev, en, en ? td->freq_offset : 0);
	if (changed & BIT(TM_CHANGED_TXPOWER))
		mt7921_tm_set_tx_power(&dev->phy);
}

static int
mt7921_tm_set_state(struct mt76_dev *mdev, enum mt76_testmode_state state)
{
	...
	if (prev_state == MT76_TM_STATE_TX_FRAMES)
		mt7921_tm_set_tx_frames(dev, false);
	else if (state == MT76_TM_STATE_TX_FRAMES)
		mt7921_tm_set_tx_frames(dev, true);
	else if (prev_state == MT76_TM_STATE_RX_FRAMES)
		mt7921_tm_set_rx_frames(dev, false);
	else if (state == MT76_TM_STATE_RX_FRAMES)
		mt7921_tm_set_rx_frames(dev, true);
	...
	if ((state == MT76_TM_STATE_IDLE &&
		...
		mt7921_tm_update_params(dev, changed);
	}
...
static int
mt7921_tm_set_params(struct mt76_dev *mdev, struct nlattr **tb,
...
static int
mt7921_tm_dump_stats(struct mt76_dev *mdev, struct sk_buff *msg)
{
	struct mt7921_dev *dev = container_of(mdev, struct mt7921_dev, mt76);
	void *rx, *rssi;
	int i;
