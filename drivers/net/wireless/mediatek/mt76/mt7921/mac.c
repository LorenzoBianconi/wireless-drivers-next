...
{
	struct mt7921_dev *dev;
#define SZ_CD (128 * 1024)
	char *dump = vzalloc(SZ_CD), *data;

	dev = (struct mt7921_dev *)container_of(work, struct mt7921_dev,
						cd.work.work);

	if (time_is_after_jiffies(dev->cd.last_activity + msecs_to_jiffies(200)))
		goto out;
	...
	while (true) {
		...
		if (!skb || data + skb->len - sizeof(*rxd) - dump > SZ_CD)
			break;
	...
	dev_coredumpv(dev->mt76.dev, dump, SZ_CD, GFP_KERNEL);
