				   ...
				   bool hdr_trans)
{
	...
	if (is_mt7921(dev)) {
		...
		ret = mt76_mcu_send_msg(dev, MCU_EXT_CMD_RX_HDR_TRANS,
			       ...
			       int cmd, void *sta_wtbl, struct sk_buff **skb)
{
	...
	struct wtbl_req_hdr hdr = {
		.wlan_idx_lo = to_wcid_lo(wcid->idx),
		.wlan_idx_hi = to_wcid_hi(wcid->idx),
		.operation = cmd,
	};
			  ...
			  bool offchan)
{
	...
	/* ht20 */
	memcpy(&sku[offset], limits->mcs[0], 8);
	offset += 8;

	/* ht40 */
	memcpy(&sku[offset], limits->mcs[1], 8);
	sku[offset + 8] = limits->mcs[0][0];
	offset += 9;
...
int mt76_connac_mcu_set_rate_txpower(struct mt76_phy *phy)
{
	...
	int i, b, idx = 0, n_chan, batch_size, band;
	struct mt76_power_limits limits;
