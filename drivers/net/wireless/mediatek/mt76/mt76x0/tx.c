		  ...
		  int pkt_len)
{
	...
	txwi = (struct mt76xx_txwi *)skb_push(skb, sizeof(struct mt76xx_txwi));
	...
	if (rate->idx < 0 || !rate->count) {
		...
		rate_ctl = mt76xx_mac_tx_rate_val(&dev->mt76, rate, &nss);
	}
	...
	mt76xx_mac_fill_txwi(txwi, skb, sta, pkt_len, nss);
	       ...
	       struct sk_buff *skb)
{
	...
	if (vif && !control->sta) {
		...
		mvif = (struct mt76xx_vif *)vif->drv_priv;
			 ...
			 struct sk_buff *skb)
{
	...
	skb_pull(skb, sizeof(struct mt76xx_rxwi));
