...
mt76x0_rx_skb_from_seg(struct mt76x0_dev *dev, struct mt76xx_rxwi *rxwi,
			void *data, u32 seg_len, u32 truesize, struct page *p)
{
	...
	true_len = mt76_mac_process_rx(dev, skb, rxwi);
				   ...
				   u32 seg_len, struct page *p)
{
	...
	rxwi = (struct mt76xx_rxwi *) data;
	data += sizeof(struct mt76xx_rxwi);
	seg_len -= sizeof(struct mt76xx_rxwi);
...
static u16 mt76x0_rx_next_seg_len(u8 *data, u32 data_len)
{
	u32 min_seg_len = MT_DMA_HDR_LEN + MT_RX_INFO_LEN +
		sizeof(struct mt76xx_rxwi) + MT_FCE_INFO_LEN;
