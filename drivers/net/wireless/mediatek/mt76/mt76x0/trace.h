...
TRACE_EVENT(mt_rx,
	TP_PROTO(struct mt76_dev *dev, struct mt76xx_rxwi *rxwi, u32 f),
	TP_ARGS(dev, rxwi, f),
	TP_STRUCT__entry(
		DEV_ENTRY
		__field_struct(struct mt76xx_rxwi, rxwi)
...
TRACE_EVENT(mt_tx,
	TP_PROTO(struct mt76_dev *dev, struct sk_buff *skb,
		 struct mt76xx_sta *sta, struct mt76xx_txwi *h),
	TP_ARGS(dev, skb, sta, h),
	TP_STRUCT__entry(
		DEV_ENTRY
		__field_struct(struct mt76xx_txwi, h)
		__field(struct sk_buff *, skb)
		__field(struct mt76xx_sta *, sta)
	),
	TP_fast_assign(
		DEV_ASSIGN;
		__entry->h = *h;
