...
struct mt76x0_dev *mt76x0_alloc_device(struct device *pdev)
{
	static const struct mt76_driver_ops drv_ops = {
		...
		.tx_complete_skb = mt76xx_tx_complete_skb,
		.tx_status_data = mt76xx_tx_status_data,
...
int mt76x0_register_device(struct mt76x0_dev *dev)
{
	...
	hw->max_rate_tries = 1;
	hw->extra_tx_headroom = sizeof(struct mt76xx_txwi) + 4 + 2;
	...
	ret = mt76_register_device(mdev, true, mt76xx_rates,
				   ARRAY_SIZE(mt76xx_rates));
	if (ret)
		return ret;

	/* disable RX LDPC */
	if (mdev->cap.has_5ghz)
		mdev->sband_5g.sband.vht_cap.cap &= ~IEEE80211_VHT_CAP_RXLDPC;
