...
void mt76x2_mac_write_txwi(struct mt76x2_dev *dev, struct mt76xx_txwi *txwi,
			   ...
			   struct ieee80211_sta *sta, int len)
{
	...
	if (wcid && (rate->idx < 0 || !rate->count)) {
		...
		txwi->rate = mt76xx_mac_tx_rate_val(&dev->mt76, rate, &nss);
	...
	mt76xx_mac_fill_txwi(txwi, skb, sta, len, nss);
}
			  ...
			  void *rxi)
{
	...
	mt76xx_remove_hdr_pad(skb, pad_len);
	...
	return mt76xx_mac_process_rate(status, rate);
}
