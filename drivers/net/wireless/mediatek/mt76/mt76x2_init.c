...
static int mt76x2_mac_reset(struct mt76x2_dev *dev, bool hard)
{
	...
	for (i = 0; i < 256; i++)
		mt76xx_mac_wcid_setup(&dev->mt76, i, 0, NULL);

	for (i = 0; i < MT_MAX_VIFS; i++)
		mt76xx_mac_wcid_setup(&dev->mt76, MT_VIF_WCID(i), i, NULL);

	for (i = 0; i < 16; i++)
		for (k = 0; k < 4; k++)
			mt76xx_mac_shared_key_setup(&dev->mt76, i, k, NULL);
...
struct mt76x2_dev *mt76x2_alloc_device(struct device *pdev)
{
	static const struct mt76_driver_ops drv_ops = {
		.txwi_size = sizeof(struct mt76xx_txwi),
...
int mt76x2_register_device(struct mt76x2_dev *dev)
{
	...
	ret = mt76_register_device(&dev->mt76, true, mt76xx_rates,
