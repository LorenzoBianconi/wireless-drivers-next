...
struct ieee80211_rate mt76xx_rates[] = {
	CCK_RATE(0, 10),
	CCK_RATE(1, 20),
	CCK_RATE(2, 55),
	CCK_RATE(3, 110),
...
EXPORT_SYMBOL_GPL(mt76xx_rates);
		   ...
		   struct ieee80211_sta *sta)
{
	...
	mt76xx_mac_wcid_setup(dev, idx, mvif->idx, sta->addr);
	mt76xx_mac_wcid_set_drop(dev, idx, false);
	for (i = 0; i < ARRAY_SIZE(sta->txq); i++)
		mt76xx_txq_init(dev, sta->txq[i]);
...
EXPORT_SYMBOL_GPL(mt76xx_sta_add);
		      ...
		      struct ieee80211_sta *sta)
{
	...
	mt76xx_mac_wcid_set_drop(dev, idx, true);
	mt76_wcid_free(dev->wcid_mask, idx);
	mt76xx_mac_wcid_setup(dev, idx, 0, NULL);
	mutex_unlock(&dev->mutex);
...
int
mt76xx_add_interface(struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
	struct mt76_dev *dev = hw->priv;
	unsigned int idx = 0;
	...
	mt76xx_vif_init(dev, vif, idx);
	return 0;
}
EXPORT_SYMBOL_GPL(mt76xx_add_interface);
...
EXPORT_SYMBOL_GPL(mt76xx_remove_interface);
...
EXPORT_SYMBOL_GPL(mt76xx_ampdu_action);
		   ...
		   struct ieee80211_key_conf *key)
{
	...
	if (!msta) {
		if (key || wcid->hw_key_idx == idx) {
			ret = mt76xx_mac_wcid_set_key(dev, wcid->idx, key);
			if (ret)
				return ret;
		}

		return mt76xx_mac_shared_key_setup(dev, mvif->idx, idx, key);
	}

	return mt76xx_mac_wcid_set_key(dev, msta->wcid.idx, key);
}
EXPORT_SYMBOL_GPL(mt76xx_set_key);
...
EXPORT_SYMBOL_GPL(mt76xx_conf_tx);
...
EXPORT_SYMBOL_GPL(mt76xx_insert_hdr_pad);
...
EXPORT_SYMBOL_GPL(mt76xx_remove_hdr_pad);

void mt76xx_remove_dma_hdr(struct sk_buff *skb)
{
	int hdr_len;

	skb_pull(skb, sizeof(struct mt76xx_txwi) + MT_DMA_HDR_LEN);
	hdr_len = ieee80211_get_hdrlen_from_skb(skb);
	if (hdr_len % 4)
		mt76xx_remove_hdr_pad(skb, 2);
}
EXPORT_SYMBOL_GPL(mt76xx_remove_dma_hdr);
...
EXPORT_SYMBOL_GPL(mt76xx_tx_complete);

void mt76xx_tx_complete_skb(struct mt76_dev *mdev, struct mt76_queue *q,
			    struct mt76_queue_entry *e, bool flush)
{
	mt76xx_remove_dma_hdr(e->skb);
	mt76xx_tx_complete(mdev, e->skb);
}
EXPORT_SYMBOL_GPL(mt76xx_tx_complete_skb);
...
EXPORT_SYMBOL_GPL(mt76xx_set_txinfo);

bool mt76xx_tx_status_data(struct mt76_dev *dev, u8 *update)
{
	struct mt76xx_tx_status stat;

	if (!mt76xx_mac_load_tx_status(dev, &stat))
		return false;

	mt76xx_send_tx_status(dev, &stat, update);
