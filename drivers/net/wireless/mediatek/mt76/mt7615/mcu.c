...
static int
mt7615_mcu_add_dev(struct mt76_phy *phy, struct ieee80211_vif *vif,
		   bool enable)
{
	struct mt7615_vif *mvif = (struct mt7615_vif *)vif->drv_priv;
	struct mt7615_dev *dev = container_of(phy->dev, struct mt7615_dev,
					      mt76);
	struct {
		struct req_hdr {
			u8 omac_idx;
	...
	return mt76_mcu_send_msg(phy->dev, MCU_EXT_CMD_DEV_INFO_UPDATE, &data,
				 sizeof(data), true);
}
...
static int
mt7615_mcu_add_bss(struct mt76_phy *phy, struct ieee80211_vif *vif,
		   struct ieee80211_sta *sta, bool enable)
{
	struct mt7615_vif *mvif = (struct mt7615_vif *)vif->drv_priv;
	struct mt7615_dev *dev = container_of(phy->dev, struct mt7615_dev,
...
static int
mt7615_mcu_wtbl_sta_add(struct mt76_phy *phy, struct ieee80211_vif *vif,
			struct ieee80211_sta *sta, bool enable)
{
	struct mt7615_vif *mvif = (struct mt7615_vif *)vif->drv_priv;
	...
	sskb = mt76_connac_mcu_alloc_sta_req(dev, &mvif->common, &msta->wcid);
	if (IS_ERR(sskb))
		return PTR_ERR(sskb);
	...
	wtbl_hdr = mt76_connac_mcu_alloc_wtbl_req(dev, &msta->wcid,
	...
	if (enable) {
		mt76_connac_mcu_wtbl_generic_tlv(dev, wskb, vif, sta, NULL,
						 wtbl_hdr);
		if (sta)
			mt76_connac_mcu_wtbl_ht_tlv(dev, wskb, sta, NULL, wtbl_hdr);
	}

	cmd = enable ? MCU_EXT_CMD_WTBL_UPDATE : MCU_EXT_CMD_STA_REC_UPDATE;
	skb = enable ? wskb : sskb;

	err = mt76_mcu_skb_send_msg(dev, skb, cmd, true);
	if (err < 0) {
...
static int
mt7615_mcu_add_sta(struct mt76_phy *phy, struct ieee80211_vif *vif,
		   struct ieee80211_sta *sta, bool enable)
{
	return mt76_connac_mcu_add_sta_cmd(phy, vif, sta, enable,
...
static const struct mt7615_mcu_ops uni_update_ops = {
	...
	.add_dev_info = mt76_connac_mcu_uni_add_dev,
	.add_bss_info = mt76_connac_mcu_uni_add_bss,
	.add_tx_ba = mt7615_mcu_uni_tx_ba,
	.add_rx_ba = mt7615_mcu_uni_rx_ba,
	.sta_add = mt76_connac_mcu_uni_add_sta,
