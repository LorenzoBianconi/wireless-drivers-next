...
int mt76xx_mac_shared_key_setup(struct mt76_dev *dev, u8 vif_idx, u8 key_idx,
				struct ieee80211_key_conf *key)
{
	enum mt76xx_cipher_type cipher;
	u8 key_data[32];
	u32 val;

	cipher = mt76xx_mac_get_key_info(key, key_data);
...
EXPORT_SYMBOL_GPL(mt76xx_mac_shared_key_setup);

int mt76xx_mac_wcid_set_key(struct mt76_dev *dev, u8 idx,
			    struct ieee80211_key_conf *key)
{
	enum mt76xx_cipher_type cipher;
	u8 key_data[32];
...
EXPORT_SYMBOL_GPL(mt76xx_mac_wcid_set_key);

void mt76xx_mac_wcid_setup(struct mt76_dev *dev, u8 idx, u8 vif_idx, u8 *mac)
{
...
void mt76xx_mac_wcid_set_drop(struct mt76_dev *dev, u8 idx, bool drop)
{
...
EXPORT_SYMBOL_GPL(mt76xx_mac_wcid_set_drop);
...
void mt76xx_txq_init(struct mt76_dev *dev, struct ieee80211_txq *txq)
{
	...
	if (txq->sta) {
		...
		sta = (struct mt76xx_sta *) txq->sta->drv_priv;
		mtxq->wcid = &sta->wcid;
	} else {
		struct mt76xx_vif *mvif;

		mvif = (struct mt76xx_vif *) txq->vif->drv_priv;
		mtxq->wcid = &mvif->group_wcid;
	}

	mt76_txq_init(dev, txq);
}
EXPORT_SYMBOL_GPL(mt76xx_txq_init);
...
EXPORT_SYMBOL_GPL(mt76xx_mac_fill_txwi);
...
EXPORT_SYMBOL_GPL(mt76xx_mac_tx_rate_val);

void mt76xx_mac_wcid_set_rate(struct mt76_dev *dev, struct mt76_wcid *wcid,
			      const struct ieee80211_tx_rate *rate)
{
	spin_lock_bh(&dev->lock);
	wcid->tx_rate = mt76xx_mac_tx_rate_val(dev, rate, &wcid->tx_rate_nss);
...
EXPORT_SYMBOL_GPL(mt76xx_mac_load_tx_status);

static int
mt76xx_mac_process_tx_rate(struct ieee80211_tx_rate *txrate, u16 rate,
...
static void
mt76xx_mac_fill_tx_status(struct mt76_dev *dev,
			  ...
			  struct mt76xx_tx_status *st, int n_frames)
{
	...
	mt76xx_mac_process_tx_rate(&rate[last_rate], st->rate,
			   ...
			   struct mt76xx_tx_status *stat, u8 *update)
{
	...
	if (msta && stat->aggr) {
		...
		mt76xx_mac_fill_tx_status(dev, &info, &msta->status,
					  msta->n_frames);
		...
		*update = 0;
		...
		mt76xx_mac_fill_tx_status(dev, &info, stat, 1);
		*update = 1;
	}
...
EXPORT_SYMBOL_GPL(mt76xx_send_tx_status);
...
EXPORT_SYMBOL_GPL(mt76xx_mac_process_rate);
