...
#define OF_EMI_RESERVED_MEMORY_STR "mediatek,leopard-N9-reserved"
	...
	vir_addr = ioremap_nocache(emi_phy_addr, emi_phy_addr_size);
	if (vir_addr != 0) {
			...
			int *actual_len, int timeout, int ep)
{
	...
	err = usb_bulk_msg(udev, pipe, data, len, actual_len, timeout);
				     ...
				     int *wait_seq)
{
	...
	*usb_hdr = cpu_to_le32(skb->len);
	...
	ret = connac_usb_mcu_bulk_msg(&dev->mt76, skb->data, skb->len, NULL,
				      1000, ep);
	if (ret < 0)
		return ret;
